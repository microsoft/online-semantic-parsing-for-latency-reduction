...
em_graph_results_re = re.compile(r'exact_match_graphs: ([0-9\.]+)')
em_graph_detok_str_results_re = re.compile(r'exact_match_graph_strings_detokenized: ([0-9\.]+)')
em_lispress_results_re = re.compile(r'exact_match_lispress: ([0-9\.]+)')
em_graph_anonym_str_results_re = re.compile(r'exact_match_graph_strings_anonymized: ([0-9\.]+)')
...
def parse_args():
    ...
    parser.add_argument('--score_item', type=str, default=None,
...
def get_score_from_log(file_path, score_name, score_item=None):
    ...
    elif score_name == 'em' or score_name == 'mod-graph.em':
        regex = em_graph_results_re
        if score_item == 'lispress':
            regex = em_lispress_results_re
        elif score_item == 'graph_detok_str':
            regex = em_graph_detok_str_results_re
        elif score_item == 'graph_anonym_str':
            regex = em_graph_anonym_str_results_re
        else:
            ...
    else:
    ...
    if results is not None:
        if 'smatch' in score_name:
            # for smatch where there is only one score
            results = results[0]
        elif 'em' in score_name:
            # for exact match there is only one score
            results = results[0]
        else:
            raise ValueError('currently only working for smatch or em where there is only one score')

    return results


def get_scores_from_beam_dir(beam_dir, data_sets, models, score_name, score_item=None):
    ...
    for dfile in os.listdir(beam_dir):
        ...
        if (data_set in data_sets) and (model_name in models) and (sname == score_name):
            score = get_score_from_log(os.path.join(beam_dir, dfile), score_name, score_item)
            results_dict.setdefault(model_name, {})[data_set] = score    # could be None

    return results_dict


def collect_final_scores(checkpoint_folder, data_sets, models, score_name, score_item=None):
    beam_sizes = []
    ...
    for bs in beam_sizes:
        ...
        results_dict = get_scores_from_beam_dir(beam_dir, data_sets, models, score_name, score_item)
...
if __name__ == '__main__':
    ...
    beam_results = collect_final_scores(args.checkpoints, args.data_sets, args.models, args.score_name, args.score_item)
    # beam_results is a dictionary, with key levels beam_size -> model_name -> data_set -> score
