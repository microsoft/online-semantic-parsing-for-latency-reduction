...
"""Truncate a source utterance to a certain length, and modify the target with _COPY_(src_pointer) actions
...
def get_prefix(utterance, perc=50):
    ...
    return prefix, truncate_len, total_len
...
if __name__ == '__main__':
    ...
    with open(src_file, 'r') as f, open(tgt_file, 'r') as ft, open(out_file, 'w') as g, open(out_tgt_file, 'w') as g2:
        for line, line_tgt in tqdm(zip(f, ft)):
            if line.strip():
                assert line_tgt.strip()

                context = line.strip().split('__User')[:-1]
                utterance = line.strip().split('__User')[-1]
                prefix, truncate_len, total_len = get_prefix(utterance.split()[:-1], perc=perc)
                if truncate_len == total_len:
