...
GOLD_LISPRESS_PATH = DATA_DIR / 'processed' / 'smcalflow2.0' / \
    'act-top_src-ct1-npwa_tgt-cp-str' / 'oracle' / "valid.lispress"
...
def drive_policy(
    ...
    final_graphs = extract_api_subgraphs(
        final_prediction, names=policy.api_names) if final_prediction is not None else []
...
def main(
    ...
    # # checkout incremental parsing results and execution process
    # scored_all_subgraphs, scored_api_subgraphs, scored_graph, selected_subgraphs, selected_api_subgraphs = \
    #     check_incremental_predictions(ps['354'][1].beam[0], API_P50_DICT, threshold)
    # for prefix_len in range(len(ps['354'])):
    #     # this will print out the parsing and selection results
    #     scored_all_subgraphs, scored_api_subgraphs, scored_graph, selected_subgraphs, selected_api_subgraphs = \
    #         check_incremental_predictions(ps['354'][prefix_len].beam[0], API_P50_DICT, threshold)
...
def pretty_lispress(lispress_str):
    from dataflow.core.lispress import parse_lispress
    from dataflow.core.lispress import render_pretty

    return render_pretty(parse_lispress(lispress_str))


def check_incremental_predictions(prediction_beam, API_P50_DICT, threshold=-1.0):
    # e.g. input: ps['354'][11].beam[0], API_P50_DICT, threshold
    api_names = tuple(API_P50_DICT.keys())
    scored_graph = prediction_beam.to_graph()
    scored_api_subgraphs = []
    for subgraph in extract_api_subgraphs(scored_graph.graph, names=api_names):
        score = LogProbThresholdPolicy(threshold=-1.0, api_names=api_names)._score_subgraph(subgraph, scored_graph)
        scored_api_subgraphs.append((subgraph, graph_to_lispress(subgraph), score))

    scored_all_subgraphs = []
    for subgraph in extract_api_subgraphs(scored_graph.graph, names=None):
        score = LogProbThresholdPolicy(threshold=-1.0, api_names=api_names)._score_subgraph(subgraph, scored_graph)
        scored_all_subgraphs.append((subgraph, graph_to_lispress(subgraph), score))

    print()
    print('[parsed graph (lispress form)]:')
    print(pretty_lispress(graph_to_lispress(scored_graph.graph)))

    print()
    print('[selected subgraphs (lispress form)]:')
    selected_subgraphs = list(filter(lambda x: x[2] >= threshold, scored_all_subgraphs))
    for g, l, s in selected_subgraphs:
        print('-' * 30)
        print(pretty_lispress(l))
        print('score:', s)

    print()
    print('[selected API subgraphs (lispress form)]:')
    selected_api_subgraphs = list(filter(lambda x: x[2] >= threshold, scored_api_subgraphs))
    for g, l, s in selected_api_subgraphs:
        print('-' * 30)
        print(pretty_lispress(l))
        print('score:', s)
    print()

    return scored_all_subgraphs, scored_api_subgraphs, scored_graph, selected_subgraphs, selected_api_subgraphs
