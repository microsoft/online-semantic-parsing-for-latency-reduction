...
def is_graph_subset(graph_set1, graph_set2):
    found = 0
    for graph1 in graph_set1:
        for graph2 in graph_set2:
            if graph1.is_identical(graph2):
                found += 1
                break

    if found == len(graph_set1):
        return True
    return False


def is_graph_in(graph, graph_set):
    for g in graph_set:
