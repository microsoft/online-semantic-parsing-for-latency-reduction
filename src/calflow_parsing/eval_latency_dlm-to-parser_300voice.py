...
# PREDICTIONS_PATH = PREDICTIONS_DIR / "beam1" / 'valid-prefixallt.parsing-from-dlm_checkpoint_best-beam5.hypos.json'
PREDICTIONS_PATH = PREDICTIONS_DIR / "beam1" / 'valid-prefixallt.parsing-from-dlm_checkpoint_best-beam1.hypos.json'
GOLD_LISPRESS_PATH = DATA_DIR / 'processed' / 'smcalflow2.0' / 'act-top_src-ct1-npwa_tgt-cp-str' / 'oracle' / "valid.lispress"
VOICE_DURATIONS_PATH = DATA_DIR / 'smcalflow2.0_voice300' / 'mapped_val_300.jsonl'
...
class DLMBeamItem:
    ...
    parsing_prediction: Prediction


@dataclass(frozen=True)
class DLMBeamList:
    beam: List[DLMBeamItem]
...
class LogProbThresholdPolicy(Policy):
    ...
    def _extract_score_weight_merge_subgraph(
            self,
            dlm_step_scores: List[List[float]],
            parser_scored_graphs: List[List[ScoredGraph]],
            merge=True,    # wether to merge subgraphs and combine scores from different beams
            num_beams=None,    # how many beams to look at (default is all)
            ) -> List[Tuple[Graph, float]]:
        """Extract subgraphs, score subgraphs, weight subgraphs according to beam, and merge subgraphs
        across beams.
        """
        scored_subgraphs: List[Tuple[Graph, float]] = []    # collected subgraphs and their scores
        if num_beams is None:
            num_beams = len(dlm_step_scores)
        for dlm_beam_scores, dlm_beam_parse in zip(dlm_step_scores[:num_beams], parser_scored_graphs[:num_beams]):
            for parser_beam_scored_graph in dlm_beam_parse:
                for subgraph in extract_api_subgraphs(parser_beam_scored_graph.graph, self.api_names):
                    # score of the subgraph (a single number to summarize)
                    score = self._score_subgraph(subgraph, parser_beam_scored_graph)
                    # weight by the dlm beam score; here we use the length normalized which is used by decoding
                    # score += np.mean(dlm_beam_scores)
                    score += sum(dlm_beam_scores)    # not normalize with length

                    # add the subgraph to the bag -> if there is already a same one, we merge them directly
                    if merge:
                        num_current_subgraphs = len(scored_subgraphs)
                        for i in range(num_current_subgraphs):
                            if scored_subgraphs[i][0].is_identical(subgraph):
                                # found the same subgraph already collected -> merge to update score
                                scored_subgraphs[i][1] = np.logaddexp(scored_subgraphs[i][1], score)    # add in prob space
                                break
                            else:
                                # the subgraph is new -> add to the collection
                                scored_subgraphs.append([subgraph, score])

                        # NOTE when the length is 0, it does not enter the for loop -> it will never update the list from above!
                        if num_current_subgraphs == 0:
                            scored_subgraphs.append([subgraph, score])

                    # add the subgraph to the bag -> duplication allowed
                    else:
                        scored_subgraphs.append([subgraph, score])

        return scored_subgraphs

    def suggest_api_calls(self, state: PolicyState, prediction: Prediction) -> List[Graph]:
        dlm_step_scores = [dlm_beam.step_scores for dlm_beam in prediction.beam]    # language completion scores
        parser_scored_graphs = [[beam.to_graph() for beam in dlm_beam.parsing_prediction.beam]
                                for dlm_beam in prediction.beam]    # parser prediction graphs
        scored_api_subgraphs = self._extract_score_weight_merge_subgraph(
            dlm_step_scores,
            parser_scored_graphs,
            merge=self.merge,
...
def load_predictions(path: Path = PREDICTIONS_PATH) -> Tuple[Dict[TurnIdx, Dict[PrefixLen, Prediction]], Dict[TurnIdx, List[str]]]:
    ...
    all_predictions = {
        turn_idx: {
            int(prefix_len): DLMBeamList([
                DLMBeamItem(
                    utterance=beam_item['completion_utter_str'],
                    step_scores=beam_item['completion_step_scores'],
                    parsing_prediction=Prediction([
                        BeamItem(list(zip(
                            parsing_beam['actions'],
                            parsing_beam['positional_scores'],
                        )))
                        for parsing_beam in beam_item['parsing_results']
                    ])
                )
...
def load_voice_durations(path: Path = VOICE_DURATIONS_PATH) -> Dict[str, AsrSentence]:
    with open(path, mode='r', encoding='utf8') as voice_durations_file:
        voice_durations_json = json.load(voice_durations_file)

    voice_durations = {idx: AsrSentence.from_json(word_durations)
                       for idx, word_durations in voice_durations_json.items()}
...
def simulate_and_evaluate(
        ...
        quiet: bool = True,
        voice_durations: Dict[str, AsrSentence] = None,
) -> List[Dict[Policy, Stats]]:

    # `word_rate_model` controls whether to use real voice timing
    # NOTE `voice_durations` is always provided as of now
    if word_rate_model == 'real-voice':
        assert voice_durations is not None

    results = []
    for idx, predictions in tqdm(all_predictions.items()):

        # if provided with voice-acting timing for some utterances, filter out the rest
        if word_rate_model == 'real-voice':
            if idx not in voice_durations:
                continue
        ...
        elif word_rate_model == 'real-voice':
            sentence = voice_durations[idx]
        else:
            raise NotImplementedError
        gold_graph = lispress_to_graph(gold_lispress)

        # # debug: graphs not committed at the correct time (error identified in drive_policy)
        # if idx == '954':
        #     print()
        #     print(' '.join(utterance))
        #     print(gold_lispress)
...
def main(
        ...
        quiet: bool = True,
        merge_subgraphs: bool = True,
        num_beams: int = None,
        voice_durations_path: str = None,
):
    ...
    # load the default voice_durations data (300 utterances)
    if voice_durations_path and os.path.exists(voice_durations_path):
        voice_durations = load_voice_durations(voice_durations_path)
    else:
    ...
    api_names = tuple(API_P50_DICT.keys())
    policies = (
        LogProbThresholdPolicy(threshold, api_names=api_names, merge=merge_subgraphs, num_beams=num_beams),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default='real-voice',    # was 'char-linear'
    )
    parser.add_argument(
        '--voice-durations-path',
        type=str,
        help='voice durations path from processed ASR results of voice-acting data',
        default=VOICE_DURATIONS_PATH,    # was None
    )
    parser.add_argument(
        '--execution-time',
        type=float,
    ...
    parser.add_argument(
        '--merge-subgraphs',
        type=int,
        help='whether to merge subgraphs from different beams',
        default=1,
    )
    parser.add_argument(
        '--num-beams',
        type=int,
        help='number of beams from DLM completion to look at',
        default=1,
    )
    parser.add_argument(
        "--threshold",
    ...
    main(
        ...
        merge_subgraphs=bool(args.merge_subgraphs),
        num_beams=args.num_beams,
        voice_durations_path=args.voice_durations_path,  # NOTE always provided with default, even it does not exist
    )
