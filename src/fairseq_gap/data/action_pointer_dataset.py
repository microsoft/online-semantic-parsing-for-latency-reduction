...
from fairseq_gap.data.data_utils import collate_tokens, collate_embeddings, collate_wp_idx
...
def collate(
    ...
    if samples[0].get('target', None) is not None:
        ...
        if samples[0].get('tgt_src_pos', None) is not None:
            tgt_src_pos = merge('tgt_src_pos', left_pad=left_pad_target)
            tgt_src_pos = tgt_src_pos.index_select(0, sort_order)
        else:
            tgt_src_pos = None
    ...
    # Word-pieces
    if samples[0].get('src_wordpieces', None) is not None:
    ...
    if samples[0].get('src_wp2w', None) is not None:
...
#     # DEBUG: Inline RoBERTa
    ...
    if collate_tgt_states:
        ...
        if samples[0].get('tgt_src_cursors', None) is not None:
...
class ActionPointerDataset(FairseqDataset):
    """Dataset for graph generation with action-pointer mechanism: source is English sentences, and
    target is action sequences, along with pointer values for arc actions where the pointers are on
    the action sequence.
    ...
    Note:
        - when we are using pretrained (e.g. RoBERTa) embeddings for the source, there is no need for "src_dict".
    """
    def __init__(self, *,
                 # src
                 src_tokens=None,                       # original src tokens
                 src=None, src_sizes=None,              # indexes in the src_dict
                 src_dict=None,                         # src dictionary
                 # src fix embeddings
                 src_fix_emb_use=False,
                 src_fix_emb_dim=None,
                 src_fix_emb=None, src_fix_emb_sizes=None,
                 src_wordpieces=None, src_wordpieces_sizes=None,    # tokenized with pretrained models e.g. BART vocab
                 src_wp2w=None, src_wp2w_sizes=None,                # word piece to word map
                 # tgt
                 tgt=None, tgt_sizes=None,              # target (output side)
                 tgt_in=None, tgt_in_sizes=None,        # target (input side - could be different from output)
                 tgt_dict=None,                         # tgt dict
                 tgt_pos=None, tgt_pos_sizes=None,      # target pointers
                 # core state info
                 tgt_vocab_masks=None,
                 tgt_actnode_masks=None,                # for the valid pointer positions
                 tgt_src_cursors=None,                  # for transition based parser
                 tgt_src_pos=None,                      # for e.g. source pointer for copying (similar to "tgt_src_cursors")
        ...
        assert not left_pad_target
        # this should be fixed as in collate function, where it is by default for tgt_vocab_masks
        ...
        # graph actions state information for each tgt step
        self.tgt_vocab_masks = tgt_vocab_masks
        self.tgt_actnode_masks = tgt_actnode_masks
        self.tgt_src_pos = tgt_src_pos
        self.tgt_src_cursors = tgt_src_cursors

        # graph structure information for each tgt step
    ...
    def __getitem__(self, index):
        ...
        src_wordpieces_item = self.src_wordpieces[index].type(torch.long) if self.src_wordpieces is not None else None
        # TODO type conversion here is needed
        src_wp2w_item = self.src_wp2w[index].type(torch.long) if self.src_wp2w is not None else None
        ...
        if self.src_fix_emb_use:
            ...
            pretrained_embed_dim = self.src_fix_emb_dim
        ...
        tgt_src_pos_item = self.tgt_src_pos[index] if self.tgt_src_pos is not None else None
        ...
        # NOTE if the target side data are with CLOSE action, we can choose not to append EOS;
        #      otherwise, we choose to append EOS.
        # else:
    ...
    def supports_prefetch(self):
        return (
            ...
            and (getattr(self.tgt_in, 'support_prefetch', False) or self.tgt_in is None)
