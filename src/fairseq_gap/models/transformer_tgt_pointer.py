...
class TransformerTgtPointerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        # additional: use fixed pretrained embeddings (e.g. RoBERTa)
        parser.add_argument('--src-pretrained-emb', type=int,
                            help='src tokens: whether to use fixed pretrained (e.g. RoBERTa) embeddings to input to encoder')
        parser.add_argument('--src-pretrained-emb-dim', type=int,
                            help='Pretrained embeddings size; we project the size to model size before feeding to encoder',
                            default=768)
        parser.add_argument('--src-pretrained-emb-no-precompute', type=int,
                            help='Compute the pretrained embeddings on the fly',
                            default=False)
        parser.add_argument('--src-pool-wp2w', type=str, choices=['none', 'bot', 'top'],
                            help='src tokens: where to pool the wordpiece embeddings to words; '
                                 '(in the case of using fixed pretrained embeddings)'
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.src_pretrained_emb = args.src_pretrained_emb
        if args.src_pretrained_emb:
            self.pretrained_emb_proj = Linear(args.src_pretrained_emb_dim, embed_dim, bias=False)
    ...
    def forward(
        ...
        src_fix_emb: Optional[torch.Tensor] = None,
        src_wordpieces: Optional[torch.Tensor] = None,
        src_wp2w: Optional[torch.Tensor] = None,
        ...
        **unused
        ...
        return self.forward_scriptable(
            ...
            src_fix_emb, src_wordpieces, src_wp2w,
    ...
    def forward_scriptable(
        ...
        Returns:
            dict:
                ...
                - **encoder_states** (List[Tensor]): all intermediate
                  ...
                  Only populated if *return_all_hiddens* is True.
        """
        if not self.src_pretrained_emb:
            ...
            # use fixed pretrained (e.g. RoBERTa) embeddings; could be wordpieces instead of word
            if self.args.src_pretrained_emb_no_precompute:
                raise NotImplementedError
            else:
                x = src_fix_emb
            x = self.pretrained_emb_proj(x)    # to map the dimension of the encoder
            encoder_embedding = x.clone()
            # further process
            if self.layernorm_embedding is not None:
                x = self.layernorm_embedding(x)
            x = self.dropout_module(x)
        ...
        if self.src_pretrained_emb:
            if self.src_pool_wp2w == 'bot':
                # pool the hidden states from bpe to original word
                # TODO add a check whether the pretrained embeddings are already pooled
...
@register_model_architecture("transformer_tgt_pointer", "transformer_tgt_pointer_tiny")
def tiny_architecture(args):
    ...
    return transformer_pointer(args)
...
def transformer_pointer(args):
    ...
    # fixed pretrained embeddings and pooling
    args.src_pretrained_emb = getattr(args, 'src_pretrained_emb', 0)
    # args.src_pretrained_emb_dim = getattr(args, 'src_pretrained_emb_dim', 768)
    args.src_pretrained_emb_no_precompute = getattr(args, 'src_pretrained_emb_no_precompute', 0)
...
@register_model_architecture("transformer_tgt_pointer", "transformer_tgt_pointer_bart_large_arch")
...
@register_model_architecture("transformer_tgt_pointer", "transformer_tgt_pointer_bart_base_arch")
...
@register_model_architecture("transformer_tgt_pointer", "transformer_tgt_pointer_6x6x4hx512x1024")
def transformer_pointer_architecture(args):
    args.encoder_embed_path = getattr(args, "encoder_embed_path", None)
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 512)
    args.encoder_ffn_embed_dim = getattr(args, 'encoder_ffn_embed_dim', 1024)
    args.encoder_layers = getattr(args, 'encoder_layers', 6)
...
@register_model_architecture("transformer_tgt_pointer", "transformer_tgt_pointer_6x6x8hx512x2048")
def transformer_pointer_architecture(args):
    args.encoder_embed_path = getattr(args, "encoder_embed_path", None)
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 512)
    args.encoder_ffn_embed_dim = getattr(args, 'encoder_ffn_embed_dim', 2048)
...
@register_model_architecture("transformer_tgt_pointer", "transformer_tgt_pointer_3x3x4hx256x512")
def transformer_pointer_architecture(args):
    args.encoder_embed_path = getattr(args, "encoder_embed_path", None)
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 256)
    args.encoder_ffn_embed_dim = getattr(args, 'encoder_ffn_embed_dim', 512)
...
@register_model_architecture("transformer_tgt_pointer", "transformer_tgt_pointer_2x2x4hx128x512")
def transformer_pointer_architecture(args):
    args.encoder_embed_path = getattr(args, "encoder_embed_path", None)
