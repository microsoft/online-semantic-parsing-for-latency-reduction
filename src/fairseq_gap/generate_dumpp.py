...
from fairseq_gap.utils import (post_process_action_pointer_prediction,
                               post_process_action_pointer_prediction_scp,
                               clean_pointer_arcs,
    ...
    def save(self):
        ...
        for n in range(self.nbest):
            ...
            with open(f'{dfile_path}.actions_nopos', 'w') as fid1, \
                    ...
                    open(f'{dfile_path}.step_scores', 'w') as fid4:
                for sid in sample_ids:
                    ...
                    fid4.write(
                        "{}\n".format(' '.join(map(str, results[sid][n]['positional_scores'])))
                    )
        # Write source sentences
        with open(f'{file_path}.src', 'w') as fid:
            for sid in sample_ids:
    ...
    def save_scp(self):
        ...
        for n in range(self.nbest):
            ...
            with open(f'{dfile_path}.actions_nopos', 'w') as fid1, \
                    open(f'{dfile_path}.actions_pos', 'w') as fid2, \
                    open(f'{dfile_path}.actions', 'w') as fid3, \
                        open(f'{dfile_path}.actions_scp', 'w') as fid4, \
                            open(f'{dfile_path}.actions_src_pos', 'w') as fid5, \
                                open(f'{dfile_path}.step_scores', 'w') as fid6:
                ...
                for sid in sample_ids:
                    ...
                    fid4.write(
                        "{}\n".format(' '.join(results[sid][n]['actions_scp']))
                    )
                    fid5.write(
                        '{}\n'.format(' '.join(map(str, results[sid][n]['actions_src_pos'])))
                    )
                    fid6.write(
                        "{}\n".format(' '.join(map(str, results[sid][n]['positional_scores'])))
                    )
...
def main(args):
    ...
    # TODO _model_args type and structures changed
    ...
    if not hasattr(_model_args.criterion, 'shift_pointer_value'):
        _model_args.criterion.shift_pointer_value = 1
    # ==================================================================================
    ...
    if args.sacrebleu:
        ...
        Cfg = namedtuple('cfg', ['pad', 'eos', 'unk'])
        cfg = Cfg(tgt_dict.pad(), tgt_dict.eos(), tgt_dict.unk())
        scorer = bleu.Scorer(cfg)
    num_sentences = 0
    has_target = True
        ...
        for sample in t:
            ...
            for i, sample_id in enumerate(sample['id'].tolist()):
                ...
                for j, hypo in enumerate(hypos[i][:args.nbest]):
                    ...
                    elif 'scp' in _model_args.task.arch:
                        actions_nopos, actions_pos, actions_src_pos, actions_scp, actions = \
                            post_process_action_pointer_prediction_scp(hypo, tgt_dict, sample['src_sents'][i])
                    ...
                    if 'scp' in _model_args.task.arch:
                        # with src copy mechanism
                        examples.append({
                        ...
                        result_hypos.setdefault(sample_id, []).append(
                            {
                                'actions_nopos': actions_nopos,
                                'actions_pos': actions_pos,
                                'actions': actions,
    ...
    if 'scp' in _model_args.task.arch:
        # with src copy mechanism
        examples.save_scp()
        json.dump(result_hypos, open(f'{args.results_path}.hypos.json', 'w'))
    else:
        # with no src copy mechanism
        examples.save()
        json.dump(result_hypos, open(f'{args.results_path}.hypos.json', 'w'))

    print(f'| All beam results with probabilities saved to {args.results_path}.hypos.json')
