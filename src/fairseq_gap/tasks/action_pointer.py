...
from fairseq_gap.data.action_pointer_dataset import ActionPointerDataset
from fairseq_gap.data.data_utils import load_indexed_dataset
from fairseq_gap.actions_preprocess.action_data_binarize import (
    ActionStatesBinarizer,
    binarize_actstates_tofile_workers,
    load_actstates_fromfile
)
from fairseq_gap.binarize import binarize_file
...
def load_action_pointer_dataset(data_path, emb_dir, split, src, tgt, src_dict, tgt_dict, tokenize, dataset_impl,
    ...
    # src: numerical values encoded by a dictionary, although not directly used if we use e.g. RoBERTa embeddings
    ...
    if src_fix_emb_use:
        src_fixed_embeddings = load_indexed_dataset(embfile_prefix + f'{src}.embs', None, dataset_impl)
        src_fixed_embeddings_sizes = src_fixed_embeddings.sizes
        # src: wordpieces
        src_wordpieces = load_indexed_dataset(embfile_prefix + f'{src}.wordpieces', None, dataset_impl)
        src_wordpieces_sizes = src_wordpieces.sizes
        # src: wordpieces to word map
        src_wp2w = load_indexed_dataset(embfile_prefix + f'{src}.wp2w', None, dataset_impl)
        src_wp2w_sizes = src_wp2w.sizes
    else:
        src_fixed_embeddings = None
        src_fixed_embeddings_sizes = None
        src_wordpieces = None
        src_wordpieces_sizes = None
        src_wp2w = None
        src_wp2w_sizes = None
    ...
    # tgt: actions (encoded by a vocabulary), action pointers, and actions states information
    tgt_actstates = load_actstates_fromfile(filename_prefix + tgt, tgt_dict, dataset_impl)

    # build dataset
    dataset = ActionPointerDataset(src_tokens=src_tokens,
                                    src=src_dataset,
                                    src_sizes=src_sizes,
                                    src_dict=src_dict,
                                    ...
                                    src_fix_emb_dim=src_fix_emb_dim,
                                    src_wordpieces=src_wordpieces,
                                    src_wordpieces_sizes=src_wordpieces_sizes,
                                    src_wp2w=src_wp2w,
                                    src_wp2w_sizes=src_wp2w_sizes,
                                    # tgt
                                    ...
                                    tgt_vocab_masks=tgt_actstates.get('tgt_vocab_masks', None),
                                    tgt_actnode_masks=tgt_actstates.get('tgt_actnode_masks', None),
                                    tgt_src_cursors=tgt_actstates.get('tgt_src_cursors', None),
                                    tgt_src_pos=tgt_actstates.get('tgt_src_pos', None),
...
@ register_task('action_pointer')
class ActionPointerParsingTask(FairseqTask):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--data-valid', type=str, default=None,
                            help='data path for validation data, if different from training')
        parser.add_argument('--emb-dir-valid', type=str, default=None,
                            help='pre-trained src embeddings directory for validation data, if different from training')
        ...
        # fixed src embeddings
        parser.add_argument('--src-fix-emb-use', default=0, type=int,
                            help='whether to use fixed pretrained (e.g. RoBERTa) contextual embeddings for src')
        parser.add_argument('--src-fix-emb-dim', default=768, type=int,
                            help='Pretrained embeddings size; we project the size to model size before feeding to encoder')

    def __init__(self, args, src_dict=None, tgt_dict=None):
        super().__init__(args)
        self.args = args            # NOTE to account for changes after fairseq v0.10.2
        ...
        assert self.args.target_lang == 'actions'

        # default to use the same directory for validation and training data
        self.args.data_valid = getattr(args, 'data_valid', None) or getattr(args, 'data', None)
        self.args.emb_dir_valid = getattr(args, 'emb_dir_valid', None) or getattr(args, 'emb_dir', None)
    ...
    def setup_task(cls, args, **kwargs):
        ...
        logger.info('| [{}] dictionary: {} types'.format(args.target_lang_nopos, len(tgt_dict)))
    ...
    def split_actions_pointer(cls, actions_filename):
        ...
        def peel_pointer(action, pad=-1):
            if action.startswith(SpecialSymbols.LA) or action.startswith(SpecialSymbols.RA):
        ...
        with open(actions_filename, 'r') as f, \
            ...
            for line in f:
                if line.strip():
                    line_actions = line.strip().split(cls.word_sep)
    ...
    @classmethod
    def preprocess_action_files(cls, actions_filenames, **kwargs):
        """Preprocess action files so that the resulted action files can be used for tgt dictionary construction.
        The default is to peel off all the target-side edge pointer values.

        Other potential preprocessing includes reformulating the actions, e.g. add source copy actions and src pointer,
        and get the file with copy actions without src pointer for the target dictionary. For different preprocessing
        needs, overwrite this method.

        The resulted file for building dictionary has extension name 'actions_nopos'.

        Args:
            actions_filenames ([type]): [description]
        """
        cls.split_actions_pointer_files(actions_filenames)
    ...
    def build_generator(self, args, model_args):
        if getattr(args, 'score_reference', False):
            ...
            if 'graph' in model_args.task.arch:
                from fairseq_gap.sequence_generator_graph import SequenceGenerator
            else:
                from fairseq_gap.sequence_generator import SequenceGenerator
