...
BOOL_TENSOR_TYPE = torch.bool    # NOTE does not work for torch version < 1.2
...
class SequenceGenerator(object):
    def __init__(
        ...
        self.copy_vocab_index = tgt_dict.indices[SpecialSymbols.COPY]
    ...
    def generate(
        ...
        **kwargs
        ...
        if self.match_source_len:
            ...
            # max_len = min(src_len * 5,    # the max ratio for train, dev and test is around 3 (8 for SMCalFlow2.0 valid)
            #               # exclude the EOS marker
            #               model.max_decoder_positions() - 1)
            # model.max_decoder_positions() is 1024 by default; it also limits the max of model's positional embeddings

            max_len = min(max(src_len * 5 + 100, 320),    # the max ratio for SMCalFlow2.0 valid is around 8, and abs max is 161
                          # exclude the EOS marker
                          model.max_decoder_positions() - 1)
            # model.max_decoder_positions() is 1024 by default; it also limits the max of model's positional embeddings
        ...
        src_max_len = encoder_outs[0]['encoder_out'][0].size(0)
        ...
        if run_amr_sm:
            ...
            # length should be bsz * beam_size
            for i in new_order:
                sm = InterfaceMachine(src_tokens=sample['src_sents'][i])
        ...
        def finalize_hypos(step, bbsz_idx, eos_scores, unfinalized_scores=None):
            ...
            # src pointers
            src_pointers_clone = src_pointers.index_select(0, bbsz_idx)
            src_pointers_clone = src_pointers_clone[:, 1:step + 2]    # skip the first index, EOS
            # =============================================================
            ...
            for i, (idx, score) in enumerate(zip(bbsz_idx.tolist(), eos_scores.tolist())):
                ...
                def get_hypo():
                    ...
                    return {
                        ...
                        'pointer_src': src_pointers_clone[i],
                    }
                    # ===========================================================
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            # ========== this is moved close to the bottom of the for loop =============================
            ...
            # NOTE the valid vocab mask is on the original vocab ->
            #      the extended copy positions are handled insided the model output
            allowed_mask = tokens.new_zeros(valid_bbsz_num, self.vocab_size, dtype=BOOL_TENSOR_TYPE)
            allowed_mask_with_copy = tokens.new_zeros(valid_bbsz_num, self.vocab_size + src_max_len, dtype=BOOL_TENSOR_TYPE)
            ...
            if amr_state_machines is not None:
                for i, j in enumerate(valid_bbsz_idx):
                    ...
                    # =====================================================================
                    # check for src copy and extend allowed src copy positions to match the output distribution size
                    # NOTE there are padding values in the src copy distribution, but those should have -inf log probability
                    #      from the model output distribution
                    if self.copy_vocab_index in vocab_ids_allowed:
                        vocab_ids_allowed.remove(self.copy_vocab_index)
                        vocab_ids_allowed = list(vocab_ids_allowed) + list(range(self.vocab_size, self.vocab_size + src_max_len))
                    else:
                        vocab_ids_allowed = list(vocab_ids_allowed)

                    allowed_mask_with_copy[i, vocab_ids_allowed] = 1
                    # =====================================================================
                ...
                allowed_mask_with_copy[:, self.unk] = 0
            else:
                ...
                allowed_mask_with_copy.fill_(1)
                allowed_mask_with_copy[:, self.pad] = 0
                allowed_mask_with_copy[:, self.unk] = 0
                allowed_mask_with_copy[:, self.tgt_dict.bos()] = 0
            ...
            lprobs[~allowed_mask_with_copy] = -math.inf
            ...
            if modify_arcact_score:
                ...
                arc_action_ids = list(set().union(*[canonical_act_ids[act] for act in [SpecialSymbols.LA, SpecialSymbols.RA, '>LA(root)']
            ...
            # ========== source pointer values decoding ==========

            # record src pointer values
            if src_pointers is None:
                src_pointers = torch.zeros_like(tokens).fill_(-1)
            ...
            lprobs_buf = lprobs.new(bsz * beam_size, self.vocab_size + src_max_len).fill_(-math.inf)
            ...
            if step < max_len:
                ...
                if prefix_tokens is not None and step <= prefix_tokens.size(1):
                    ...
                    if partial_prefix_mask.any():
                        ...
                        partial_scores, partial_indices, partial_beams = self.search.step(
                            step,
                            lprobs.view(bsz, -1, self.vocab_size + src_max_len),
                ...
                print(f'reached max len:', max_len)    # max_len is changing with batches
                print(f'max src len:', src_len)
                # breakpoint()
            ...
            if len(finalized_sents) > 0:
                ...
                if src_pointers is not None:
                    src_pointers = src_pointers.view(bsz, -1)[batch_idxs].view(new_bsz * beam_size, -1)
                # =================================================================
            ...
            active_mask_selected, active_hypos = torch.topk(
            ...
            # reorder target pointer values and scores
            src_pointers[:, :step + 2] = torch.index_select(src_pointers[:, :step + 2], dim=0, index=active_bbsz_idx)


            # ========== reorder incremental state in decoder =============================================
            # NOTE this needs to happen before we run the state machine, as the source side information with
            #      correct correspondence to the beams is needed, to recover the correct src_pointer value
            #      by considering the padding in batch
            if step < max_len:    # before the last step in iteration
                # reorder incremental state in decoder
            ...
            if amr_state_machines is not None:
                ...
                # number of paddings in the src batch on the left
                num_src_left_paddings = encoder_outs[0]['encoder_padding_mask'][0].sum(1)
                assert len(num_src_left_paddings) == tokens_buf.size(0)

                # add and apply new action tokens to state machine
                for i, (sm, act_id, act_pos, n_src_pad, is_valid) in enumerate(
                    zip(amr_state_machines,
                        tokens_buf[:, step + 1],
                        tgt_pointers[:, step + 1],
                        num_src_left_paddings,
                        valid_bbsz_mask)
                    ):
                    ...
                    # COPY actions
                    if act_id >= len(self.tgt_dict):
                        act = SpecialSymbols.COPY
                        # breakpoint()
                        act_src_pos = act_id - len(self.tgt_dict)    # NOTE this includes padding positions in batch
                        # recover the src position after removing the left padding in the batch
                        act_src_pos -= n_src_pad
                        act_src_pos = act_src_pos.item()
                    else:
                    ...
                    # # debug: close even within an unfinished string -> no such cases
                    # if sm.string_within and act_id == self.eos:
                    #     breakpoint()

                    sm.apply_action_and_update_states(act, act_pos.item(), act_src_pos)

                    # ========== dealing with output the actions and src pointers in the beam records =====
                    if act_id >= len(self.tgt_dict):
                        # update the action id in the dictionary to be the COPY action
                        tokens_buf[i, step + 1] = self.copy_vocab_index

                        # record the source pointer value
                        src_pointers[i, step + 1] = act_src_pos
                    # =====================================================================================
    ...
    def _decode_one(
        ...
        if attn is not None:
            if isinstance(attn, dict):
                attn = attn['attn']
            attn = attn[:, -1, :]
        # NOTE the scp model already outputs log probabilities, concatenated from vocab and src
        # probs = model.get_normalized_probs(decoder_out, log_probs=log_probs)
        if log_probs:
            probs = decoder_out[0]
        else:
            probs = torch.exp(decoder_out[0])
        probs = probs[:, -1, :]
        return probs, attn
