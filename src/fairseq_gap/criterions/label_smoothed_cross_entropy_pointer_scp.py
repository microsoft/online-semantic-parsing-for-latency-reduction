...
@register_criterion('label_smoothed_cross_entropy_pointer_scp')
class LabelSmoothedCrossEntropyPointerSrcCopyCriterion(LegacyFairseqCriterion):
    ...
    def __init__(self, args, task):
        ...
        self.copy_vocab_index = task.tgt_dict.indices[SpecialSymbols.COPY]
        self.vocab_size = len(task.tgt_dict)
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        ...
        lprobs = net_output[0]    # the output is already log probs
        lprobs = lprobs.view(-1, lprobs.size(-1))
        target = model.get_targets(sample, net_output).view(-1, 1)    # size (bsz * tgt_max_len, 1)
        target_scp = sample['net_input']['tgt_src_pos'].view(-1, 1)    # size (bsz * tgt_max_len, 1)

        # when it's COPY action, the target is from the copy distribution
        target = torch.where(target == self.copy_vocab_index, target_scp + self.vocab_size, target)
