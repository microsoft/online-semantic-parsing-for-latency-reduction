...
arch=transformer_tgt_pointer_scp_2x2x4hx128x512
arch_tag=_2x2x4hx128x512
...
if [[ $tgt_src_copy_layers == "0 1" ]]; then
    scp_lay="all"
else
    scp_lay=""
    for n in $tgt_src_copy_layers; do
        [[ $n < 0 || $n > 1 ]] && echo "Invalid 'tgt_src_copy_layers' input: $tgt_src_copy_layers" && exit 1
        scp_lay=$scp_lay$(( $n + 1 ))
    done
fi
