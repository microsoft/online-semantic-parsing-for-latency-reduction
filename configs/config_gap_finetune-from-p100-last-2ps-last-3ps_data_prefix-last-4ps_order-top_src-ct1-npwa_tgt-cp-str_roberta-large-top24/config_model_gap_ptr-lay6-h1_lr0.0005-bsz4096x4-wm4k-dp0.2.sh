...
##### load data config
config_data=configs/config_data_prefix/config_data_prefix-last-4ps_order-top_src-ct1-npwa_tgt-cp-str_roberta-large-top24.sh
...
# finetune from previous model on 90% and 100% data, which was finetuned from model on 100% data
starting_model=$SAVEDIR/exp_finetune-from-p100-last-2ps_prefix-last-3ps_order-top_src-ct1-npwa_tgt-cp-str_roberta-large-top24_gap_vmask1_shiftpos1_ptr-lay6-h1_scp-lay5-h1_dec-emb-sha0/models_ep50_seed42_fp16-lr0.0001-mt4096x4-wm4000-dp0.2/checkpoint50.pt
TRAIN_ARGS="${TRAIN_ARGS}
    --finetune-from-model $starting_model
    "
finetune_flag=_finetune-from-p100-last-2ps-last-3ps
