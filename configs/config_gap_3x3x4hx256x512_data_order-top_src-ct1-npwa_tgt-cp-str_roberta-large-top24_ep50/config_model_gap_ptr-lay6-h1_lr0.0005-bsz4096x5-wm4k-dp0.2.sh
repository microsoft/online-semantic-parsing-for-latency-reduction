...
##### load data config
config_data=configs/config_data/config_data_order-top_src-ct1-npwa_tgt-cp-str_roberta-large-top24.sh
...
arch=transformer_tgt_pointer_scp_3x3x4hx256x512
arch_tag=_3x3x4hx256x512
...
if [[ $tgt_src_copy_layers == "0 1 2" ]]; then
    scp_lay="all"
else
    scp_lay=""
    for n in $tgt_src_copy_layers; do
        [[ $n < 0 || $n > 2 ]] && echo "Invalid 'tgt_src_copy_layers' input: $tgt_src_copy_layers" && exit 1
        scp_lay=$scp_lay$(( $n + 1 ))
    done
fi

# set the experiment directory name
expdir=exp_${data_tag}_gap${arch_tag}_vmask${tgt_vocab_masks}_shiftpos${shift_pointer_value}
