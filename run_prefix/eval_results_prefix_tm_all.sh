...
if [[ -z $2 ]]; then
    save_file="results_collection/compute_all_tm_results_from_${exp_prefix}.txt"
else
    save_file=$2
fi


echo "Computed tm results to be saved at: [$save_file]"
...
if [[ $exp_dir == *"order-bot"* ]]; then
    ...
    for checkpoints_folder in "${model_folders[@]}"; do
        ...
        bash run_prefix/eval_results_prefix_tm.sh $config_model
...
eval_tm_all |& tee $save_file    # dump to file and print to console
# eval_tm_all &> $save_file        # only dump to file
# less $save_file
