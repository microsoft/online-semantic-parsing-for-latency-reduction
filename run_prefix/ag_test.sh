...
if [[ ! -s $results_prefix.actions ]]; then
...
if [[ ! -s $results_prefix.lispress ]]; then
...
if [[ ! -s $results_prefix.mod-graph.em ]]; then
...
##### Evaluation: graph tuple match, precision recall and f1 (similar to SPICE)

if [[ ! -s $results_prefix.tm ]]; then
...
# tuple match for the graph with anonymous copy action tailored for the prefix

if [[ ! -s $results_prefix.mod-graph.tm ]]; then
...
##### Evaluation: graph API call subgraph match, precision recall and f1

if [[ ! -s $results_prefix.apim ]]; then

python -m calflow_parsing.api_match \
    --dataset $dataset \
    --test_actions $results_prefix.actions \
    --out_file $results_prefix.apim \
    --gold_actions $gold_actions

fi

echo -e "\n[API match results with full graph:]"
cat $results_prefix.apim

# API subgraph match for the graph with anonymous copy action tailored for the prefix

if [[ ! -s $results_prefix.mod-graph.apim ]]; then

python -m calflow_parsing.api_match \
    --dataset $dataset \
    --test_actions $results_prefix.actions \
    --out_file $results_prefix.mod-graph.apim \
    --gold_actions $PREFIX_ALL_ORACLE_FOLDER/${data_split}.actions

fi

echo -e "\n[API match results with graph copy modified with prefix:]"
cat $results_prefix.mod-graph.apim
