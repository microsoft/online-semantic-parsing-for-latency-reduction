...
do
    ...
    if [[ -s $MODEL_FOLDER/beam${beam_size}/${validset}_checkpoint${model_epoch}.actions ]]; then
        ...
        if [[ ! -s $MODEL_FOLDER/beam${beam_size}/${validset}_checkpoint${model_epoch}.tm ]]; then
            . $dir/ag_test_spice.sh "" $validset
        else
            echo "$MODEL_FOLDER/beam${beam_size}/${validset}_checkpoint${model_epoch}.tm already exists --- do not recompute"
            cat "$MODEL_FOLDER/beam${beam_size}/${validset}_checkpoint${model_epoch}.tm"
            echo
            echo "$MODEL_FOLDER/beam${beam_size}/${validset}_checkpoint${model_epoch}.mod-graph.tm already exists --- do not recompute"
            cat "$MODEL_FOLDER/beam${beam_size}/${validset}_checkpoint${model_epoch}.mod-graph.tm"
        fi
